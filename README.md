...
## Features
 ...
 - **Autonomous Agent Planning:** LangGraph-powered workflow for multi-step reasoning, tool orchestration, bounded execution, planning refinement, and graceful fallbacks.

- **Deterministic Budget Engine:** INR-first budgeting with Indian number formatting, dynamic hotel/meal/activity/transport costs, optional currency conversion, and budget constraint                                         validation.

- **Multi-Source Data Integration:** Integrates weather, places, currency, and transport intelligence using external APIs with caching, timeout handling, and feasibility checks.

- **Production-Grade Infrastructure:** Uses cached LangGraph instances, lightweight request caching, connection timeouts, exponential backoff, and optional MongoDB persistence for                                                resilient operation.

- **Observability & Debugging:** Tracks LLM latency, tool execution time, workflow duration, structured request logs, service health, and database status for easier debugging and                                           monitoring.
## Tech Stack
